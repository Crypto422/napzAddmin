...
import { deleteImageData } from '../../ImageUploadCRUD';
import {
  ref,
  deleteObject,
...
const ImageUploadPage: React.FC<Props> = ({ className }) => {
  ...
  const handleChange = (value: any, id: any) => {
    ...
    if (value) {
      ...
      imageItem.image_name = imgName[id];
      imageItem.image_id = imgId[id];
    ...
    if (value) {
      for (j = 0; j < tempItems.length; j++) {
        ...
        if (uploadState[id] === false) {
          checkState[id] = value;
          tempItems[j].image_name = imgName[id];
          tempItems[j].image_id = imgId[id];
  ...
  const handleUpload = () => {
    let uid = getUid();
    setImageItems([{ ...data, user_id: user.user_auth_id, imageFile: imageFile, isNew: true, id: uid }, ...imageItems]);
    checkState[uid] = false;
    setCheckState(checkState);
    uploadState[uid] = true;
    setUploadState(uploadState);
  ...
  const handleDelete = () => {
    ...
    for (var i = 0; i < selectedImages.length; i++) {
      let imageId = selectedImages[i].image_id;
      let imageName = selectedImages[i].image_name;
  ...
  const handleOneDelete = (imageId: string, imageName: string, id: any) => {
    let tempItem = { "image_id": imageId, "image_name": imageName, "id": id };
  ...
  const deleteItem = (imageId: number, imageName: string, id: any) => {
    setDeleting(true);
