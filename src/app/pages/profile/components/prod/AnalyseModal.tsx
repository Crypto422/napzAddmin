...
import { createAnalysedData } from "../../AnalysedDataCRUD";
...
const AnalyseModal = (props: any) => {
    ...
    const uploadJsonDataToStorage = (json: any) => {
        let fileName = InitialData.image_name + ".json";
    ...
    const uploadDataToDb = (downloadURL: any) => {
        const item = {
            image_data_id: InitialData.id,
            data_url: downloadURL,
            json_file_name:InitialData.image_name,
            user_id: user.id
        };
        try {
            createAnalysedData(item)
            .then((res) => {
                let {data} = res;
                setAnalyse(true);
                setAnalysedId(data.id);
                setLoading(false);
            })
            .catch((error) => {
                console.log(error);
                            ...
                            <div className="row gy-5 gx-xl-8">
                                <div className="col-12  col-xl-7" style={{ display: "flex", justifyContent: 'center', alignItems: 'center' }}>
                                    <div className=' position-relative' style={{ width: '100%', maxWidth: 'fit-content' }}>
                                        <img id='imageid' style={{maxHeight: '500px', opacity: "100%", width: 'inherit', borderRadius: '3%' }} src={InitialData.image_url} alt='UploadImage' />
                                    </div>
                                </div>
