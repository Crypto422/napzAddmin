...
import { register, getUserByEmail } from '../../redux/AuthCRUD'
...
export function Registration() {
      ...
      setTimeout(() => {
        createUserWithEmailAndPassword(auth, values.email, values.password)
          .then((response) => {
            ...
            let name = `${values.lastname}${values.firstname}`;
            let email = user.email || "";
            let password = values.password;
            let user_auth_id = user.uid;
            let auth_provider = 'local';
            register({ email, name, username, user_auth_id, auth_provider, password })
              .then((response) => {
                let { data } = response;
                setLoading(false)
                setSubmitting(false)
                dispatch(Auth.actions.register(data.accessToken, data.refreshToken))
                SendSocketCreateMsg(data);
              })
              .catch(() => {
  ...
  const SendSocketCreateMsg = (user: any) => {
    let msg: any = {
      type: 'CreateUser',
      body: `${user.id}`
    }
        ...
        <div className={checkgoogle ? `overlay-wrapper` : ''}>
          ...
          <div className='row fv-row mb-6'>
            <div className='col-xl-6'>
              <label className='class="form-label fw-bolder text-light fs-6'>First name</label>
              <input
            ...
            <div className='col-xl-6'>
              <label className='class="form-label fw-bolder text-light fs-6'>Last name</label>
              <input
